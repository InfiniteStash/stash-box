...
func (qb *UserQueryBuilder) FindByIds(ids []uuid.UUID) ([]*User, []error) {
	query := "SELECT users.* FROM users WHERE id IN (?)"
...
func (qb *UserQueryBuilder) GetRolesByIds(ids []uuid.UUID) ([]UserRoles, []error) {
	joins := UserRoles{}
