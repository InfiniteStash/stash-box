...
func (qb *SceneQueryBuilder) SearchScenes(term string) ([]*Scene, error) {
	query := `
        ...
        WHERE (
			setweight(to_tsvector('simple', COALESCE(scene_date, '')), 'A') ||
			setweight(to_tsvector('english', studio_name), 'B') ||
			setweight(to_tsvector('english', COALESCE(performer_names, '')), 'B') ||
			setweight(to_tsvector('english', scene_title), 'C')
